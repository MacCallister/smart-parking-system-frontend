...
import { Camera, AlertCircle, CheckCircle, Clock, RefreshCw, Search, X } from 'lucide-react';
...
type ViolationStatus = 'new' | 'reviewed' | 'resolved';
...
const App = () => {
    ...
    useEffect(() => {
        ...
        // Search filter
        if (searchTerm) {
            filtered = filtered.filter((v: Violation) =>
        ...
        // Status filter
        if (statusFilter !== 'all') {
            filtered = filtered.filter((v: Violation) => v.status === statusFilter);
        }

        // Camera filter
        if (cameraFilter !== 'all') {
            filtered = filtered.filter((v: Violation) => v.camera_id === cameraFilter);
        }
    ...
    const updateStatus = async (id: string, newStatus: ViolationStatus) => {
        try {
    ...
    // Get unique cameras
    const cameras = ['all', ...new Set(violations.map((v: Violation) => v.camera_id))];
    ...
    const stats = {
        ...
        new: violations.filter((v: Violation) => v.status === 'new').length,
        noPlate: violations.filter((v: Violation) => v.plate_text === 'no_plate_detected' || v.plate_text === 'unreadable').length,
        detected: violations.filter((v: Violation) => v.plate_text && v.plate_text !== 'no_plate_detected' && v.plate_text !== 'unreadable').length
    };
